...
func IR2STI (IR: [Float], sampleRate: Int) -> Float{
    ...
    var IRLenth: Int = IR.count + PaddingLength
    var paddedIR: [Float] = Array(repeating: 0, count: PaddingLength) + IR
    for i in 0...6{
        ...
        for j in 0...13{
            ...
            var Fm_cycles: Int = Int(mf[j] * Float(IRLenth) / Float(sampleRate))
            // number of samples to use for each modulation frequency
            var Fm_len: Int = Int (Float(Fm_cycles) * Float(sampleRate) / mf[j])
            var MTF_num: Float = 0
            var MTF_i: Float = 0
            var MTF_den: Float = 0
            var factor: Float = Float.pi * -2 * mf[j]
            for k in 0...(Fm_len-1){
                var phase :Float = factor * Float(k) / Float(sampleRate)
...
print (IR2STI(IR: [1,2,3,4,5,5,6,6,7,7,8], sampleRate: 1))